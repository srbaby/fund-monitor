...
// 东方财富基金行情基地址（direct 模式用，官方净值主源）。返回 JSON。
const EM_BASE = "https://fundmobapi.eastmoney.com";
...
// 不在表中的产品不参与基准代理估值
//
// 字段名是 legs 不是 equity：混合品种只挂权益腿（债券+现金那 75-85% 日波动 1-2bp，
// 摊到净值 <0.02%，建模没有收益），但纯债品种整只都是债券腿，挂 000012 国债指数。
// 一张表里两种腿共存，叫 equity 会误导后来人以为纯债那行填错了。
const BENCHMARK_PROXY = {
  // ── 枷锁层 F4 ──
  "160622": { legs: [{ idx: "000832", w: 0.15 }] },       // 鹏华丰利: 中证转债×15%+全债×85%
  "110027": { legs: [{ idx: "000300", w: 0.15 }] },       // 易方达回报: HS300×15%+全债×85%
