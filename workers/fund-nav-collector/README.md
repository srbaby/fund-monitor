...
- 当上游返回的日期等于最近一条已存在历史记录的日期、而该记录缺少某只基金时，只回填这条历史记录；不放宽当天记录判据，不追溯更早日期，也不另发请求。
- 回填条目标记为 `backfilled`，不写抢先时刻 `at`，因此读端点的 `firstCount` 不会把它算作当晚竞速参与者。
- 历史回填只搭尚未到齐的采集跳，不会因为历史缺口单独唤醒上游；当前基金全部到齐后立即早退，不再请求上游。
...
- 历史记录发生回填时写对应的 `nav:{date}`；若它是当前/上一窗口，还同步更新对应指针。
...
当前测试共 15 项，覆盖真实写盘路径、早退、基金增删、双源抢先、历史回填、官方净值窗口和错误可见性。
