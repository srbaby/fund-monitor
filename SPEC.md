...
**文件上限：9个，不可突破。** 禁止为任何新功能新建文件。唯一例外：`logger.js` 为诊断用途破例存在，见第十四节。
...
└── js/
    ...
    └── logger.js      # 诊断层：云端操作日志，破例文件，见第十四节
```

脚本加载顺序固定：`config → logger → store → data → engine → ui → interact → main`
...
`logger.js` 是唯一破例的第10个文件，长期保留，用于排查数据异常问题。

### 用途

记录所有本地写入操作和云端推送操作，包含触发函数名、时间戳、完整数据快照，写入独立的 Log Gist（`fm_log.json`），最多保留1000条，滚动覆盖旧记录。未配置 LogGistID 时 `fmLog` 静默跳过，无任何副作用。

### 配置方式

云同步配置格式为 `GistID,Token,LogGistID`（第三段为 Log Gist ID）。Log Gist 在 GitHub 单独新建，初始文件名 `fm_log.json`，内容 `[]`。LogGistID 与主 Gist 共用同一个 Token。

### 接入点

| 文件        | 函数                | 记录内容                |
| ----------- | ------------------- | ----------------------- |
| store.js    | `saveFunds`         | 写入的基金列表          |
| store.js    | `savePe`            | 写入的完整 PE 对象      |
| store.js    | `saveHoldingsData`  | 写入的持仓数据          |
| store.js    | `saveSellPlan`      | 写入的降权预案          |
| store.js    | `savePrioritySell`  | 写入的优先卖出代码      |
| store.js    | `clearPrioritySell` | 清除操作                |
| store.js    | `importSnapshot`    | pull 写入的 f 和 p 字段 |
| interact.js | `syncCloud doPush`  | 推送的完整 payload      |

### 停用方法

1. 删除 `logger.js` 文件
2. 删除 `index.html` 中 `<script src="js/logger.js"></script>`
3. 删除 store.js 和 interact.js 中所有 `fmLog(...)` 调用行

### 复用方法

重新加入 `logger.js`，在 `index.html` 中 `config.js` 之后恢复加载，在各接入点加回 `fmLog(...)` 即可。`STORE_LOG_GIST_ID`、`loadLogGistId`、`saveLogGistId` 均定义在 `logger.js` 内，不迁入 config.js 或 store.js。
