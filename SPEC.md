...
**云推送不挂广播**：`syncCloud` 由 interact 层在配置变更时显式调用，`LOCAL_CONFIG` 订阅不触发云推送。`FUNDS` 同时被净值刷新触发，挂订阅会每分钟产生无意义 API 调用。正确调用点：`addFund`、`delFund`、`saveHoldings`、`confirmPe`，各自按类型显式调用 `syncCloud("push_pe_now" / "push_config")`。
...
```
confirmPe()
    → savePe() → 广播 LOCAL_CONFIG → UI_updateLocalConfig()
    → syncCloud("push_pe_now")

saveHoldings() / addFund() / delFund()
    → saveHoldingsData() / saveFunds() → 广播 LOCAL_CONFIG / FUNDS
...
```
syncCloud("pull")
    → cloudFetchPe() + cloudFetchConfig()   // 两文件并行拉取
    → importPeSnapshot()                    // 比对 p，相同跳过
    → importSnapshot()                      // 比对 f/h/s/pr，相同跳过
    → 有变更时 refreshData()
```
...
```
fm_pe.json     ← p 字段，PE 定锚（高频，每日）
fm_config.json ← f / h / s / pr 字段（低频）
fm_log.json    ← 操作日志
```
...
- `importSnapshot` / `importPeSnapshot` 内部直接写 localStorage 跳过各封装函数的广播，各自最后统一广播，避免双重渲染
...
- [ ] `syncCloud` 在配置变更的 interact 函数中显式调用，未挂在广播订阅；PE 变更用 `push_pe_now`，持仓/列表变更用 `push_config`
...
破例的第10个文件，长期保留。记录本地写入和云端推送操作（时间戳、客户端标识、函数名、数据快照），写入主 Gist 的 `fm_log.json` 文件，最多200条滚动覆盖（基于实际使用数据调整，200条约覆盖10天）。未配置 Gist 时静默跳过。

**配置**：与主 Gist 共用 `GistID,Token`，日志写入 `fm_log.json`。首次使用前须在主 Gist 手动添加 `fm_log.json`，初始内容为 `[]`。

**接入点**：store.js 的 `saveFunds`、`savePe`、`saveHoldingsData`、`saveSellPlan`、`savePrioritySell`、`clearPrioritySell`、`importSnapshot`、`importPeSnapshot`；interact.js 的 `syncCloud_push_pe`、`syncCloud_push_config`。
