...
- `pe_nightly.py` 跨午夜日期检查：旧逻辑 `last_date != today_bj` 在槽位延迟跨午夜（含手动补触发）会 exit 1 拒写当晚有效锚。**2026-06-14 已修**：凌晨8点前执行接受前一交易日数据（`data_current = last_date ∈ {今天, 昨天}`）。CF Worker 准点触发后该场景已极少，此修主要保住午夜后手动补触发的安全（应急 SOP 红灯窗口由此从"24:00 硬红线"放宽为"次日08:00前可补写"）。

**验证期SOP（2026-06-11 改版：CF Worker 哨兵替代每晚人工核对）**

1. ~~每晚22:35人工核对~~ → **已自动化**：CF Worker `pe-night-trigger` 三槽自愈（北京20:30/21:30/22:00），22:00 哨兵每晚必推 Bark——🟢 = 锚已写+当晚errPp，无需任何操作；🔴 = 前两槽未成（推送同时已自动补触发最后一次），应急窗口延至次日08:00前（pe_nightly 跨午夜容错支持补写前一交易日锚）。
