> **归档说明（2026-07-21）**：本文件是被取代前的原始全文。
> 被取代的是 §3.2 的 getCurrentPE 条——那套机制在 D-006 前后已改为旁路引擎百分位，
> 文档当时没跟上，一直烂到本次审查才发现。缘由见 [DECISIONS D-016](../DECISIONS.md#d-016)。
> 其余章节当时仍然有效，现行版本在 [../03-实现约束.md](../03-实现约束.md)。
...
**估算持久化（直连模式专用，[D-014](../DECISIONS.md#d-014)）**：
- **localStorage** `fm_est_cache_v1`：每次成功拉取估算后写入，当日 TTL 18h，非当日 2h。
- **Gist** `fm_est.json`：收盘后 fire-and-forget 推送一次，由 `_maybePushEstToGist` 触发，`fm_est_gist_date_v1` 标记每日去重。
