...
main setInterval → refreshData()
    → fetchIndices()              // 网关 /v1/indices ｜ 直连 TX_BASE（由 DATA_MODE 决定，见 D-013）
    → fetchOfficialData()         // 网关 /v1/funds/official ｜ 直连 TX_BASE（整组）
...
> - `"direct"`：浏览器直连腾讯 `TX_BASE`，**单请求同时覆盖官方净值 + 盘中估算两链**
>   （`jj{code}` 拼接），指数单独单请求（`sh000xxx` / `hkHSI`）；返回 GBK，前端 `TextDecoder("gbk")` 解码。
