...
> **四组数据体系**（[D-020](DECISIONS.md#d-020) 2026-07-21 建立）：
> ① 实时指数 ② 官方净值 ③ 盘中估算（全源已死，结构保留） ④ 业绩基准代理（计算自 ① + 产品库，`DATA_SOURCE_SWITCH` 开关控制）
>
> **官方净值：唯一来源是采集器 KV**（[D-023](DECISIONS.md#d-023) G 节）。
...
>
> **数据源双模只管盘中数据（估算、指数）**（[D-013](DECISIONS.md#d-013)）：`DATA_MODE` 切两条链路——
...
>
> **官方净值夜间采集器**（[D-023](DECISIONS.md#d-023)）：`workers/fund-nav-collector/`，
> 带 Cron 的 Worker，9:00–24:00 每分钟一跳（cron 已由 [D-027](DECISIONS.md#d-027) 更新），`Promise.allSettled`
> 并行打东财与腾讯，逐只只收 `officialAt === 今日` 的净值，**先到先得**（`src` / `at` 写入后不可变），
> 两源都给出当日数据时按**完成时间**选先到的，平局归东财（[D-026](DECISIONS.md#d-026)，取代 D-023 A 节"东财平局胜"）。
...
> **直连模式下不跳过**：腾讯盘后返回 0 → 自动回退 localStorage/Gist 缓存（[D-014](DECISIONS.md#d-014)），估值列保留不消失。
> 回退来的数据**按其自带的 `estimateAt` 判新旧**，非当日即挂「陈旧」小字，卡片与表格两个视图都挂——
> 沿用旧数据可以，静默沿用不行（[D-018](DECISIONS.md#d-018)，与 [D-001](DECISIONS.md#d-001) 同向）。
...
> **本节写「前端必须遵守什么」，不写「为什么当初这么定」。** 理由的单一出处是
> [DECISIONS.md](DECISIONS.md)，网关内部实现与性能预算在 `workers/fund-market-api/README.md`。
> 三处各司其职，避免同一段理由抄三遍后各自漂移。
>
> ⚠️ **但操作性规则必须留在这里**，哪怕看起来与决策日志重复——2026-07-20 的事故正是因为
> 「拿不到新数据就用旧的」这条规则只活在代码里、没进正文（见 [D-001](DECISIONS.md#d-001)）。
> 删规则和删理由是两件事，**只删后者**。
...
- **`stale` = 网关的 last-known-good 回退**：两条线路都取不到时网关返回上次那份完整好数据，
  ...
  **前端必须渲染它并明确标记陈旧**——指数走 `mode:"stale"`（`idx-bar` 显示「行情暂断 · 显示 HH:MM 数据」），
  ...
  KV 回退的取舍与事故经过见 [D-001](DECISIONS.md#d-001)，网关侧字段与降级行为见 `workers/fund-market-api/README.md`。
...
- **in-flight 请求去重**：同一 cacheKey 的并发请求只打一次上游，纯网关内部行为，前端无感。
  实现与理由见 [D-011](DECISIONS.md#d-011)。
- **指数组主线路是腾讯，不是东方财富**：PE bar 锚定 `getAnchorPE`，默认解析到 `getEnginePE1`（1.0 总市值路，见 [D-017](DECISIONS.md#d-017)），唯一输入是沪深300
...
（原 `SYS_CONFIG` 聚合对象已于 2026-07-21 删除，业务数字改为 config.js 顶层裸常量，见 [D-020](DECISIONS.md#d-020) 代价栏。）
