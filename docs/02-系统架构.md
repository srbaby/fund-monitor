...
```
main setInterval(60s) → refreshData()
    → fetchIndices()              // 网关 /v1/indices ｜ 直连：腾讯单源（D-020）
    → fetchOfficialData()         // 网关 /v1/funds/official ｜ 直连：东财(主)→腾讯(备)（D-020）
...
> 带 Cron 的 Worker，19:00–23:00 每分钟一跳抢发布 + 每小时一跳兜底，`Promise.allSettled`
