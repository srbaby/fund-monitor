...
>
> **数据源双模**（[D-013](DECISIONS.md#d-013)）：`config.js` 的 `DATA_MODE` 切换两条链路——
> - `"gateway"`：走网关三端点，主备 + KV last-known-good 在网关内。
> - `"direct"`：浏览器直连。**指数单源腾讯**——新浪备源因不返回 CORS 头在浏览器侧取不到，
>   代码已于 D-020 删除，**本文档任何地方都不得再写"指数有主备"**（此处曾长期写着"备新浪"，
>   D-023 时修正）。净值先问采集器，缺口再走主东财 / 备腾讯（D-020、[D-023](DECISIONS.md#d-023)）。
>   返回格式：腾讯 GBK（`TextDecoder("gbk")` 解码），东财 JSON，采集器 JSON。
>   字段产物与网关逐字段对齐，下游 `fetchSingleFund` / `setIndices` / `setQQIndex` 无感。
>   `workers/fund-market-api/` 网关此时不被前端使用，仅作备选保留。
>
> **官方净值夜间采集器**（[D-023](DECISIONS.md#d-023)）：`workers/fund-nav-collector/`，
> 带 Cron 的 Worker，19:00–23:00 北京每分钟一跳，`Promise.allSettled` 并行打东财与腾讯，
> 逐只只收 `officialAt === 今日` 的净值，**先到先得**（`src` / `at` 写入后不可变），存 KV。
> 前端 `_fetchOfficialDirect` 先读 `/v1/nav/today`，缺口才走直连链，整体不可用则完全退回直连。
> 存在理由是两条**浏览器侧无解**的约束：东财前端直连被 `ErrCode:61136` 拦（仅服务端调得通），
> 且无人开着看板时浏览器根本不会去取。因此官方列**可能混源**，见 `03-实现约束.md`。
